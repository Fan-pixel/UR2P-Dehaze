...
class HazeRemovalNet(BaseNetwork):
    ...
    def forward(self, x_0, require_paras=False, use_guided_filter=False):
        ...
        #x_0 = WTConv2d(in_channels = 3,out_channels = 3)
        #x_0 = WTConv2d(in_channels=1, out_channels=1, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1')
        # 将图像转换为PyTorch张量，并添加批次维度（PyTorch期望的）
        # transform = transforms.Compose([
        #     transforms.ToTensor(),
        #     transforms.Normalize(mean=[0.5], std=[0.5])  # 根据您的需要调整均值和标准差
        # ])
        # image_tensor = transform(x_0).unsqueeze(0)  # 添加批次维度

        # # 确保图像张量在正确的设备上（CPU或GPU）
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        #wt_conv2d = WTConv2d(in_channels=256, out_channels=256, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1').to(device)
        ...
        wtconv = DepthwiseSeparableConvWithWTConv2d(in_channels=32, out_channels=32).to(device)
        out1 = wtconv(out1)
        layer_2 = self.pretrained.layer2(layer_1)
        layer_3 = self.pretrained.layer3(layer_2)
        layer_4 = self.pretrained.layer4(layer_3)
        #print("layer_4",layer_4.shape)#layer_4 torch.Size([8, 384, 8, 8])
        ...
        #layer_4_rn = wt_conv2d(layer_4_rn).to(device)
